...
//    @Scheduled(fixedDelay = 5000L)
     ...
     *
     */
    public void sendUsingReceiveQueue() throws IOException {
        Message message = new Message(UUID.randomUUID().toString(), "hello rabbit!!!");
        log.info("send msg: {} to server using async template ", message);
        org.springframework.amqp.core.Message messageToSend = MessageBuilder
                .withBody(objectMapper.writeValueAsBytes(message))
                .setContentType(MediaType.APPLICATION_JSON_VALUE)
                .setCorrelationId(message.getId().getBytes())
                .build();

        org.springframework.amqp.core.Message response =
                amqpTemplate.sendAndReceive(ASYNC_EXCHANGE_NAME, ASYNC_ROUTING_KEY, messageToSend);

        byte[] responseBody = response.getBody();
        Message responseMessage = objectMapper.readValue(responseBody, Message.class);
        log.info("receive response Message （: = {}", responseMessage);
    }
